<template>
                    ...
                    <v-container>
                      <v-row>
                        <v-text-field
                          ...
                          v-model="employeeId"
                        ></v-text-field>
                        <v-btn @click="submitEmployeeId" color="orange" dark solo x-large>Confirm</v-btn>
                      </v-row>
                      ...
                      <v-row justify="center">
                        <v-dialog v-model="uploadModalOpen" max-width="400" >
                          <template v-slot:activator="{ on, attrs }">
                            <v-btn @click="clearEmployeeId" elevation="24" color="orange" icon rounded fab plain v-bind="attrs" v-on="on">
                              <v-icon size="35" left>mdi-qrcode-scan</v-icon>
                              <span class="underline-animation" style="font-family: Evogria; font-size: 1.8rem; font-weight: bold;">SCAN your QR to Confirm</span>
                            </v-btn>
                          </template>
                          <v-card class="orange-border" style="font-family: Evogria;" >
                            <v-card-title  @mouseover="hovered = true" @mouseleave="hovered = false" :class="{ 'orange-text': hovered }">SCAN YOUR QR CODE</v-card-title>
                            ...
                            <v-card-text class="text-center"  @mouseover="hovered = true" @mouseleave="hovered = false" :class="{ 'orange-text': hovered }">
                              <v-btn @click="cancelUpload" class="underline-animation">
                                <v-icon size="22" left>mdi-camera</v-icon>
                                Use Camera</v-btn>
                            </v-card-text>
                          </v-card>
                          <v-card class="orange-border" id="qr-code-full-region"></v-card>
  ...
  export default {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8000/api/confirm-employee-registration', {
      ...
      confirmButton: 'orange-button',
        ...
        Swal.fire({
          title: 'Success!',
          html: `${response.data}`, // Access the remaining_tickets property from the response data object
          icon: 'success',
          customClass: {
      confirmButton: 'orange-button',
       }
