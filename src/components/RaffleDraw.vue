<template>
    <div class="row d-flex justify-content-right align-items-end" style="min-height: 90vh;">
      <div class="g-pa-40 g-mb-30 vacant_positions"> 
        <img v-if="showImage && currentImage === '1st Round.png'" src="@/assets/1st Round.png" class="img-fluid winner-list-img">
        <img v-if="showImage && currentImage === '2ndRound.png'" src="@/assets/2ndRound.png" class="img-fluid winner-list-img">
        <v-form>
            ...
            <div class="scene perspective-on">
              <div id="lottery" class="center">
                <div class="lottery-machine">
                  ...
                  <div class="lottery-right-door" :class="{ 'animation': isrightDoor }"></div>
                  <div class="lottery-left-door" :class="{ 'animation': isleftDoor }"></div>
                  <div class="lottery-lift" :class="{ 'animation': lift }"></div>
                  <div class="lottery-ball" :class="{ 'animation-up': ballSelected, 'animation-roll': ballRoll}"  :key="ballSelectedKey"></div>
                </div>
                <div id="ball-place"></div>
              </div>
            </div>
            <v-btn class="orange-border underline-animation-black black-font glass-animation" 
              id="play" 
              @click="pullLeverAndShowImage" 
              style="font-family: Evogria" color="orange" dark solo>Draw</v-btn>
          </v-row>
        </v-form>
      </div>
    </div>
...
pullLeverAndShowImage() {
      ...
      if (this.clickCount >= 11) {
        this.currentImage = '2ndRound.png';
        this.showImage = true;
      } else {
        this.showImage = true;
      }
...
mounted(){
    ...
    this.fetchData().then(() => {
        class LotteryMachine {
            constructor(participants, rafflePrizes, winnerList, currentPrizeIndex, opt = {}) {
                this.currentPrizeIndex = currentPrizeIndex;
            ...
            rollBallOut(parent, placeForBall, animationOpt, callback, raffleWinner, rafflePrizes, currentPrizeIndex, winnerList){ 
                ...
                function moveBallOut() {
                    ...
                    const spacingInterval = 4; // Adjust spacing every 5 balls
                    const spacingOffset = 290; // Initial spacing offset
                    const leftSpacing = endCoords.left + Math.floor(ball_count / spacingInterval) * spacingOffset + -300;
                    ...
                    return new Promise(resolve => {
                        gsap.to(animatedBall, 1, {
                            ...
                            onComplete: () => {
                                ...
                                const paperHeight =48; // Set the desired height for the paper
                                ...
                                defaults = { startVelocity: 20, spread: 360, ticks: 90, zIndex: 0 };
                                ...
                                console.log("currentPrize, ", currentPrizeIndex);
        ...
        function getNextRaffleParticipants(prizeCode){
            ...
            console.log("testNexxtRaffle", prizeCode);
            let participants = [];
        ...
        axios.get('http://127.0.0.1:8000/api/raffle-participants')
        .then(response => {
            if (response.data && response.data["raffleParticipants"]) {
            this.raffleParticipants = response.data["raffleParticipants"];
            this.rafflePrizes = response.data["rafflePrizes"];
            }
            var raffleParticipants = this.raffleParticipants;
            var rafflePrizes = this.rafflePrizes;
        ...
        console.log("datsaDulo:", data);
        // var svg = d3.select('#chart')
        //     .append("svg")
        //     .data([data])
        //     .attr("width",  w + padding.left + padding.right)
        //     .attr("height", h + padding.top + padding.bottom)
        //     .style({"font-weight":"bold", "font-family": 'Evogria'});
        ...
        function spin(d){
            ...
            var  ps       = 360/data.length,
                pieslice = Math.round(1440/data.length),
                 rng      = Math.floor((Math.random() * 10000) + 10000);
            ...
            vis.transition()
                ...
                .each("end", function(){
                    ...
                    var selectedName = data[picked].full_name;
                    var ticket_num = data[picked].full_name;
                    Swal.fire({
                        title: "Congratulations!",
                        html: `Name: ${selectedName}<br>Prize: ${ticket_num}`,
                        // icon: "success",
                        timer: 3000,
                        showCancelButton: false,
                        showConfirmButton: false,
                    ...
                    confetti({
                        ...
                        shapeOptions: {
                            image: [
                                {
                                    src: "images/confetti/ingco_tool.png",
                                    width: 32,
                                    height: 32,
                                },
                                {
                                    src: "images/confetti/ingco_wrench.png",
                                    width: 32,
                                    height: 32,
                                },
                                // {
                                //     src: "images/confetti/ingco_generator.png",
                                //     width: 32,
                                //     height: 32,
                                // },
                            ],
                    ...
                    $('#noDataPlaceholder').hide();
                    $('#winnerData').append(`
                            <tr>
                                <td class="g-color-black" style="font-family:'Evogria'">${data[picked].full_name}</td>
                                <td class="g-color-black" style="font-family:'Evogria'">${data[picked].full_name}</td>
                                <td class="g-color-black" style="font-family:'Evogria'">${data[picked].store_branch_ecommerce_name}</td>
                            </tr>
                        `
                    );
                    winnerIDS.push(data[picked].value)
                    /* Comment the below line for restrict spin to sngle time */
                    let formData = new FormData();
                    formData.append('winnerID',data[picked].value)
                    formData.append('multiple',0)
        ...
        //     .attr("cy", 0)
        //     .attr("r", 60)
        //     .style({"fill":"#f69f1a","cursor":"pointer"});
        // //spin text
        ...
        //     .text("SPIN")
        //     .style({"font-weight":"bold", "font-size":"30px", "font-family": 'Evogria','fill':'black'});
        ...
        $('.fullscreen_btn').click(function(e){
            window.location.href = 'winnerDraw?pc='+ rafflePrizes.code
            // window.open('http://127.0.0.1:8001/winnerDraw?promo_code=ABC123')
        })

        $('#draw_type').change(function(e){
            let value = $(this).val();
            // If Spin the wheel
            if(value == 1){
                $('#chart').removeClass('d-none');
                $('#wheel_col').removeClass('col-lg-4');
                $('#wheel_col').addClass('col-lg-11');
                $('#lottery_col').addClass('col-lg-12');
...
.lottery-machine .lottery-ball::before {
    ...
    color: #f69f1a; /* Color of the question mark */
